...
public class Main {
            ...
            try {
                ...
                switch (task) {
                    ...
                    case 2:
                        ...
                        Task2.save(fileName);
                        ArrayList<Tour> tours = Task2.load(fileName);//"Task2.txt"
                        System.out.println("Список туров после выполнения первой части задания");
                        for (Tour tour : tours) {
                            System.out.println(tour.getTourName() + " " + tour.getHotelName() + " " +
                                    tour.getCompanyName() + " " + tour.getPrice());
                        }
                        System.out.println("\nВернуть город и сторону по наименованию тура");
                        String name = in.nextLine();
                        for (Tour tour : tours) {
                            if(!(tour.getCountryOfTourName(name).compareTo("") == 0 ))
                                System.out.println(tour.getCountryOfTourName(name));
                        }
                        tours = Task2.randomAccess(tours);
                        System.out.println("Список туров после выполнения второй части задания");
                        for (Tour tour : tours) {
                            System.out.println(tour.getTourName() + " " + tour.getHotelName() + " " +
                                    tour.getCompanyName() + " " + tour.getPrice());
                        }
                        ...
                        System.out.println("\nЕсть ли в турах позиции с одинаковыми ценниками");
                        boolean flag = false;
                        for(int i = 0; i < tours.size() - 1; i++){
                            flag = tours.get(i).compare(tours.get(i+1));
                            if(flag)
                                break;
                        }
                        if(flag)
                            System.out.println("Были найдены туры с одинаковыми ценниками");
                        else
                            System.out.println("Не было найден туров с одинаковыми ценниками");
                        break;
                    case 3:
    ...
    private static void task1(){
        try {
            ...
            f2.createNewFile();
            File f3 = new File("C:\\Users\\alexf\\Desktop\\MyFile3.txt");
            f3.createNewFile();
            ...
            System.out.println("\nУпражнение 2 ---------------------------------------------------------------");
            if (f3.isFile())
                System.out.println(f3.getName() + " является файлом, его родитель - " + f3.getParent());
            else
                System.out.println(f3.getName() + " не является файлом");
            if(dir.isDirectory())
    ...
    private static void task3Ex3(){
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(new FileInputStream("C:\\Users\\alexf\\Desktop\\OOP_lab04\\src\\A.txt"), "Cp1251"));
            ...
            in = new BufferedReader(new InputStreamReader(new FileInputStream("C:\\Users\\alexf\\Desktop\\OOP_lab04\\src\\B.txt"), "UTF-8"));
            str = in.readLine();
            ...
            try {
                ...
                switch (ex) {
                    ...
                    case 3:
                        ...
                        sObj.majeCollection();
                        sObj.readCollection();
                        ArrayList<SW> arr = sObj.getCollection();
                        System.out.println("Десериализованная коллекция обхектов:");
                        for(SW str: arr)
                            System.out.println(str.getLine());
                        break;
                }
            }
            catch (Exception e){
